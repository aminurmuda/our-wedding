...
function Adab() {
    const list = [
        ...
        { id: 4, isActive: true, message: 'Tidak menyisakan makanan yang sudah diambil', icon: mdiSilverwareForkKnife },
        { id: 5, isActive: true, message: 'Memperhatikan waktu sholat (ada masjid di dekat gedung)', icon: mdiMosqueOutline },
        { id: 6, isActive: true, message: 'Tidak mengambil foto/video mempelai tanpa izin', icon: mdiCameraOffOutline },
                ...
                {list.map(item => {
                    return (
                        <div key={item.id} className="grid-item">
                            {item.iconAlt ?
                                <div className="scale-up"><Icon size={2} path={item.icon} className="mr-0-5" /><Icon size={2} path={item.iconAlt} className="mr-0-5" /></div>
                                :
